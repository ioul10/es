"""
core/dgi_parser.py
Parser DGI (7 pages) : extrait les valeurs et les mappe sur le template fixe MCN.
...
_DGI_PREFIX = re.compile(r'^[\*\.]?\s*(I{{1,3}}|IV|V?I{{0,3}}|IX|X{{1,3}}|XIV|XV|XVI)\.\s*', re.I)
...
# ══════════════════════════════════════════════════════════════════
# ROTATION — UTILITAIRES
# ══════════════════════════════════════════════════════════════════

def _get_rotation(page) -> int:
    """
    Lit l'angle de rotation de la page en degrés (0, 90, 180, 270).
    Cherche dans plusieurs sources selon la version de pdfplumber :
      1. Attribut direct page.rotation  (pdfplumber >= 0.9)
      2. Dictionnaire PDF sous-jacent   (/Rotate)
      3. Heuristique MediaBox           (largeur >> hauteur → probablement 90°)
    """
    # 1. Attribut direct
    ...
    # 2. Objet PDF sous-jacent
    try:
        rot = page.page_obj.get('/Rotate', 0)
        return int(rot) % 360
    except Exception:
        pass

    # 3. Heuristique : page nettement plus large que haute
    try:
        if page.width > page.height * 1.3:
            return 90
    except Exception:
        pass
...
def _normalize_page(page):
    """
    Retourne la page remise d'aplomb (rotation effective = 0°).

    pdfplumber.Page.rotate(angle) applique une rotation RELATIVE antihoraire.
    Pour annuler une rotation absolue R il faut appliquer (360 - R) % 360 :
      - page à  90° → rotate(270)
      - page à 180° → rotate(180)
      - page à 270° → rotate( 90)
      - page à   0° → inchangée
    """
    rot = _get_rotation(page)
    if rot == 0:
        return page

    correction = (360 - rot) % 360
    logger.info(
        f"  page {page.page_number} : rotation={rot}° détectée "
        f"→ correction appliquée ({correction}°)"
    )
    return page.rotate(correction)
...
def _extract_section(pdf, page_indices: list, is_actif: bool = False) -> list[tuple[str, list]]:
    ...
    Gère les pages pivotées à 90°, 180° et 270°.
    """
    all_rows = []
    for idx in page_indices:
        ...
        # ── Remise d'aplomb de la page (0°, 90°, 180°, 270°) ──────
        ...
        if not good:
            # Pas de tableau structuré → tentative X/Y directe
            rows = _xy_rows(page)
            if rows:
                logger.info(f"  page {idx+1} : aucun tableau → fallback X/Y ({len(rows)} lignes)")
        ...
        if any(_has_fused(t) for t in good):
            ...
            for t in good:
                ...
                for row in t:
                    if not row or _is_index_row(row):
                        continue
                    if len(row) < 2:
                        continue
                    ...
                    if _ROMAIN_RE.match(col1.strip()):
                        ...
                        label = col1
                    if _is_rotated(label):
                        label = ''
                    if not label or len(label) < 2:
                        continue
                    if _parse(label) is not None:
                        continue

                    def gv(col):
                        if col and len(row) >= col:
                            return _parse(row[col - 1])
                        return None
...
def parse(pdf_path: str) -> dict:
    ...
    logger.info(f"  DGI Actif  : {len(actif_map)}/{len(ACTIF)} postes")
    logger.info(f"  DGI Passif : {len(passif_map)}/{len(PASSIF)} postes")
    logger.info(f"  DGI CPC    : {len(cpc_map)}/{len(CPC)} postes")
